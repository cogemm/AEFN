...
def train_one_run(run_id: int, args, hsi_pca: np.ndarray, lidar_norm: np.ndarray,
    ...
    model = HighAcc_AEFN_Net(hsi_bands=hsi_pca.shape[-1], num_classes=len(CATEGORIES), use_coordatt=bool(args.use_coordatt), use_agf=bool(args.use_agf)).to(DEVICE)
    ...
    # 保存更清晰的单图（GT / Pred 分开保存，风格同第二份代码）
