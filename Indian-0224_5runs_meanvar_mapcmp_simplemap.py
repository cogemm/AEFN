...
# ---------------------------------------------------------------------
# Extra: save a single classification map (no legend/no axes) like the
# compact GT/pred label images commonly used in HSI papers.
# ---------------------------------------------------------------------
def _hex_to_rgb(hex_color: str):
    hex_color = hex_color.lstrip("#")
    return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))

def label_to_rgb_image(label_map: np.ndarray, palette_hex):
    """Convert label map (0..K) to RGB image using given hex palette."""
    h, w = label_map.shape
    rgb = np.zeros((h, w, 3), dtype=np.uint8)
    for lab, hx in enumerate(palette_hex):
        rgb[label_map == lab, :] = np.array(_hex_to_rgb(hx), dtype=np.uint8)
    return rgb

def save_classification_map_compact(
    label_map: np.ndarray,
    out_path: str,
    palette_hex,
    upscale: int = 6,
    crop_to_labeled: bool = False,
    pad: int = 0,
):
    """
    Save a compact label map:
    - no title, no axes, no legend
    - crisp nearest-neighbor edges
    - optional upscaling for publication (default: 6×)
    - optional crop to labeled region (gt>0) for tighter framing
    """
    import numpy as _np
    from PIL import Image

    lm = label_map.astype(_np.int32)

    if crop_to_labeled:
        ys, xs = _np.where(lm > 0)
        if len(ys) > 0:
            y0, y1 = ys.min(), ys.max()
            x0, x1 = xs.min(), xs.max()
            y0 = max(0, y0 - pad); y1 = min(lm.shape[0]-1, y1 + pad)
            x0 = max(0, x0 - pad); x1 = min(lm.shape[1]-1, x1 + pad)
            lm = lm[y0:y1+1, x0:x1+1]

    rgb = label_to_rgb_image(lm, palette_hex)
    img = Image.fromarray(rgb)
    if upscale and upscale > 1:
        img = img.resize((img.size[0]*upscale, img.size[1]*upscale), resample=Image.NEAREST)
    img.save(out_path)
...
def run_five_rounds(
    ...
    # Save compact single-map images (no legend/no axes), matching the attached GT-style map.
    # 0 is background; palette uses colors[0..16] (hex).
    gt_compact_path = os.path.join(out_dir, "IndianPines_GT_compact.png")
    pred_compact_path = os.path.join(out_dir, "IndianPines_hsi_cnn_compact.png")
    save_classification_map_compact(gt_full.astype(np.int32), gt_compact_path, palette_hex=colors, upscale=6)
    save_classification_map_compact(best_result["pred_map"].astype(np.int32), pred_compact_path, palette_hex=colors, upscale=6)
