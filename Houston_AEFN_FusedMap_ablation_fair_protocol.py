...
class HighAcc_AEFN_Net(nn.Module):
    def __init__(self, hsi_bands, num_classes=15, use_coordatt: bool = True, use_agf: bool = True):
        super().__init__()
...
def train_one_run(run_id: int, args, hsi_pca: np.ndarray, lidar_norm: np.ndarray,
    ...
    print(f"[Run {run_id} | AEFN-core] OA: {core_oa * 100:.2f}%, AA: {core_aa * 100:.2f}%, Kappa: {core_kappa:.4f}")
    ...
    print(f"[Run {run_id} | AEFN+   ] OA: {plus_oa * 100:.2f}%, AA: {plus_aa * 100:.2f}%, Kappa: {plus_kappa:.4f} (EMA-eval{' on' if args.use_ema else ' off'}, TTA={plus_tta_eval})")
    ...
    if getattr(args, "save_core_plus_maps", True):
        ...
        save_map_like_demo(
            ...
            out_path=os.path.join(args.results_dir, f"Run_{run_id}_Pred_AEFN_core.png"),
            colors=COLORS,
            class_names=CATEGORIES,
        )
        ...
        save_map_like_demo(
            ...
            out_path=os.path.join(args.results_dir, f"Run_{run_id}_Pred_AEFN_plus.png"),
            colors=COLORS,
            class_names=CATEGORIES,
        )
    else:
    ...
    plot_bar_charts(core_pc, f"{run_id}_AEFN_core", args.results_dir)
    plot_bar_charts(plus_pc, f"{run_id}_AEFN_plus", args.results_dir)
...
def main():
    ...
    plot_bar_charts(core_summary["pc_mean"], "Overall_Mean_AEFN_core", args.results_dir,
                    core_summary["pc_mean"], core_summary["pc_std"])
    plot_bar_charts(plus_summary["pc_mean"], "Overall_Mean_AEFN_plus", args.results_dir,
                    plus_summary["pc_mean"], plus_summary["pc_std"])

    _print_and_save_summary("AEFN-core", core_summary, args.results_dir)
    _print_and_save_summary("AEFN+", plus_summary, args.results_dir)
    ...
    protocol_df.to_csv(os.path.join(args.results_dir, "Final_Metrics_AEFN_core_vs_plus.csv"), index=False)
    pd.DataFrame(per_run_rows).to_csv(os.path.join(args.results_dir, "PerRun_Metrics_AEFN_core_vs_plus.csv"), index=False)
